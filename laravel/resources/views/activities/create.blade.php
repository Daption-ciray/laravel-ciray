<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Yeni Aktivite Ekle') }}</h2>
    </x-slot>
    <div class="py-12">
                    ...
                    <form action="{{ route('activities.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="exercise_id" id="exercise_id"
                                ...
                                required>
                                <option value="">Seçiniz</option>
                                @foreach($exercises as $exercise)<option value="{{ $exercise->id }}">
                                {{ $exercise->name }}</option>@endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            <label for="repetition" class="block text-sm font-medium text-gray-700">Tekrar
                                Sayısı</label>
                            <input type="number" name="repetition" id="repetition" value="{{ old('repetition') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-purple-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-purple-700 transition">Kaydet</button>
                        </div>
                    </form>
                </div>
            </div>
