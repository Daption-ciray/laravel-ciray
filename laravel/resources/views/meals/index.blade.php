<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Öğünler') }}</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Yiyecek</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Öğün Zamanı
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($meals as $meal)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $meal->id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $meal->customer->name ?? '-' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $meal->food->name ?? '-' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $meal->mealtime }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $meal->like ? '👍' : '👎' }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-gray-500">Henüz öğün bulunmamaktadır.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
